...
EXERCISES.push(
  ...
  ex({ id: "high_incline_push_up", name: "高位上斜俯卧撑", englishName: "High Incline Push-up", category: exerciseSeed("push_up").category, primaryMuscles: exerciseSeed("push_up").primaryMuscles, secondaryMuscles: exerciseSeed("push_up").secondaryMuscles, movementPattern: exerciseSeed("push_up").movementPattern, equipment: [bench, stablePlatform], equipmentOptions: [[bench], [stablePlatform]], difficulty: "beginner0", contraindications: exerciseSeed("push_up").contraindications, alternatives: ["wall_push_up", "kneeling_push_up", "low_incline_push_up"], defaultRepRange: "8–15次", defaultRestSeconds: 45 }),
  ex({ id: "low_incline_push_up", name: "低位上斜俯卧撑", englishName: "Low Incline Push-up", category: exerciseSeed("push_up").category, primaryMuscles: exerciseSeed("push_up").primaryMuscles, secondaryMuscles: exerciseSeed("push_up").secondaryMuscles, movementPattern: exerciseSeed("push_up").movementPattern, equipment: [bench, box, stablePlatform], equipmentOptions: [[bench], [box], [stablePlatform]], difficulty: "beginner0", contraindications: exerciseSeed("push_up").contraindications, alternatives: ["kneeling_push_up", "push_up"], defaultRepRange: "8–15次", defaultRestSeconds: 60 }),
...
export function validateExerciseLibrary() {
  ...
  for (const exercise of EXERCISES) {
    ...
    if (["high_incline_push_up", "low_incline_push_up"].includes(exercise.id)) {
      if ((exercise.equipmentOptions || []).some(option => option.length === 1 && option.includes("bodyweight"))) {
        errors.push(`${exercise.id}: 上斜俯卧撑不能使用单独 [bodyweight] 作为完成方案`);
      }
      const requiredSupports = exercise.id === "high_incline_push_up" ? ["bench", "stable_platform"] : ["bench", "box", "stable_platform"];
      const hasSupportedOption = (exercise.equipmentOptions || []).some(option => option.some(item => requiredSupports.includes(equipmentId(item))));
      if (!hasSupportedOption) errors.push(`${exercise.id}: 缺少抬高支撑面 equipmentOptions`);
    }
    if (/incline.*push_up/.test(exercise.id) && exercise.programRole === "workset") {
      const hasRaisedSupport = (exercise.equipmentOptions || []).some(option => option.some(item => raisedSupportEquipment.has(equipmentId(item))));
      if (!hasRaisedSupport) errors.push(`${exercise.id}: 工作组上斜俯卧撑必须至少有一种抬高支撑器械方案`);
    }
